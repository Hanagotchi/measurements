...
from app.resources.rule_parser import (
    LIGHT_RULES_MAP,
    apply_light_rule,
    apply_temperature_rule,
    check_l_rule,
    check_t_rule,
    eval_deviation,
    is_daytime,
...
class ServiceTests(unittest.TestCase):
    ...
    def test_eval_multiple_light_rules(self):
        ...
        def test_fn(range, value):
            return eval_deviation(range, lambda x: apply_light_rule(x, value))

        self.assertEqual(test_fn([2, 3, 4], 10), "lower" if is_daytime() else None)
        self.assertEqual(test_fn([2, 3, 4], LIGHT_RULES_MAP[4][1][0]-1),
                         "lower" if is_daytime() else None)
        self.assertEqual(test_fn([2, 3, 4], LIGHT_RULES_MAP[4][1][0]), None)
        self.assertEqual(test_fn([2, 3, 4], LIGHT_RULES_MAP[4][1][0]+1), None)
        self.assertEqual(test_fn([2, 3, 4], 50), None)
        self.assertEqual(test_fn([2, 3, 4], LIGHT_RULES_MAP[4][1][1]-1), None)
        self.assertEqual(test_fn([2, 3, 4], LIGHT_RULES_MAP[4][1][1]), None)
        self.assertEqual(test_fn([2, 3, 4], LIGHT_RULES_MAP[4][1][1]+1), None)

        self.assertEqual(test_fn([2, 3, 4], LIGHT_RULES_MAP[3][1][0]-1), None)
        self.assertEqual(test_fn([2, 3, 4], LIGHT_RULES_MAP[3][1][0]), None)
        self.assertEqual(test_fn([2, 3, 4], LIGHT_RULES_MAP[3][1][0]+1), None)
        self.assertEqual(test_fn([2, 3, 4], 125), None)
        self.assertEqual(test_fn([2, 3, 4], LIGHT_RULES_MAP[3][1][1]-1), None)
        self.assertEqual(test_fn([2, 3, 4], LIGHT_RULES_MAP[3][1][1]), None)
        self.assertEqual(test_fn([2, 3, 4], LIGHT_RULES_MAP[3][1][1]+1), None)

        self.assertEqual(test_fn([2, 3, 4], LIGHT_RULES_MAP[2][1][0]-1), None)
        self.assertEqual(test_fn([2, 3, 4], LIGHT_RULES_MAP[2][1][0]), None)
        self.assertEqual(test_fn([2, 3, 4], LIGHT_RULES_MAP[2][1][0]+1), None)
        self.assertEqual(test_fn([2, 3, 4], 300), None)
        self.assertEqual(test_fn([2, 3, 4], LIGHT_RULES_MAP[2][1][1]-1), None)
        self.assertEqual(test_fn([2, 3, 4], LIGHT_RULES_MAP[2][1][1]), None)
        self.assertEqual(test_fn([2, 3, 4], LIGHT_RULES_MAP[2][1][1]+1), "higher")
        self.assertEqual(test_fn([2, 3, 4], 700), "higher")
